/**
 * Multi-Edit Tool — adds a separate `multi_edit` tool alongside Pi's built-in `edit` tool.
 *
 * Supports all original edit parameters (path, oldText, newText) plus:
...
export default function (pi: ExtensionAPI) {
	pi.registerTool({
		...
		label: "Multi Edit",
		description:
			"Apply one or more exact text edits, or a Codex-style patch, without replacing Pi's built-in edit tool.",
		promptSnippet:
			"Apply batched exact-text edits or a Codex-style patch across one or more files.",
		promptGuidelines: [
			"Use this tool when you need multiple exact edits in one call or need to apply a patch payload.",
			"Prefer Pi's built-in edit tool for single precise edits.",
			"Use the `multi` parameter to apply multiple edits in sequence.",
