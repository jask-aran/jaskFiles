...
const SYSTEM_PROMPT = `You are a question extractor. Given text from a conversation, extract or synthesize the questions that would let the user answer what still needs user input.
...
Rules:
- First extract explicit questions that require user input
- If there are no explicit questions, look for decisions, tradeoffs, forks, preferences, or multiple options mentioned in the text
- When such tradeoffs or options exist, synthesize concise user-facing questions that would make the next step easy
- Prefer questions like choosing between concrete options, setting priorities, or resolving a stated tradeoff
- Keep questions in the order the underlying decisions appeared
- Be concise with question text
- Include context only when it provides essential information for answering, especially when naming the tradeoff or options
- Only return {"questions": []} when the text contains neither direct questions nor actionable decisions/tradeoffs/options for the user to resolve
...
Example output:
{
  "questions": [
    ...
    {
      "question": "Should we optimize for implementation speed or long-term flexibility?",
      "context": "The last message presented a simpler option and a more extensible option."
    },
    {
...
export default function (pi: ExtensionAPI) {
	const answerHandler = async (ctx: ExtensionContext) => {
			...
			if (extractionResult.questions.length === 0) {
				ctx.ui.notify("No questions, tradeoffs, or options found in the last message", "info");
				return;
			}
	...
	pi.registerCommand("answer", {
		description: "Extract questions or decision points from last assistant message into interactive Q&A",
		handler: (_args, ctx) => answerHandler(ctx),
	});

	pi.registerShortcut("ctrl+.", {
		description: "Extract and answer questions or decision points",
